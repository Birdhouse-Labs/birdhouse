...
const PatternLibraryDialog: Component<PatternLibraryDialogProps> = (props) => {
  ...
  const [previewPattern, setPreviewPattern] = createSignal<Pattern | null>(null);
  ...
  const previewPatternHandler = async (patternId: string) => {
    const groupId = selectedGroupId();
    if (!groupId) return;

    try {
      const fullPattern = await fetchPattern(groupId, patternId, props.workspaceId);
      setPreviewPattern(fullPattern);
    } catch (_err) {
      // Silently handle error
    }
  };
  ...
  const _openEditForm = (pattern: Pattern) => {
    setEditingPattern(pattern);
    openModal(MODAL_TYPE_FORM, `edit-${pattern.id}`);
  };
    ...
    >
      <Dialog.Portal>
        ...
        >
                    ...
                    <Show when={selectedGroupId()}>
                      {(groupId) => (
                        <GroupView
                          ...
                          onPreviewPattern={previewPatternHandler}
          ...
          {/* Preview Dialog */}
          <Show when={previewPattern()}>
            {(pattern) => (
              <Dialog
                open={true}
                onOpenChange={(open) => {
                  if (!open) setPreviewPattern(null);
                }}
              >
                <Dialog.Portal mount={document.body}>
                  ...
                  >
                    <div class="px-6 py-3 border-b bg-surface-raised border-border flex-shrink-0 flex items-center justify-between">
                      <Dialog.Label class="text-lg font-semibold text-heading">Preview: {pattern().title}</Dialog.Label>
                      <Dialog.Close class="text-text-muted hover:text-text-primary transition-colors">
                        <X size={20} />
                      </Dialog.Close>
                    </div>
