// ABOUTME: Agent top bar experiment - Context Donut as Button
// ABOUTME: Context usage donut with IconButton-style interactions (hover, press, click-only popover)
...
export const metadata = {
  id: "52",
  title: "Context Donut as Button",
  description: "Context usage donut with IconButton-style interactions (hover, press, click-only popover)",
};
...
const Experiment52 = () => {
  ...
  const ContextUsageIndicator = () => {
    ...
    return (
      <Tooltip openDelay={999999} closeDelay={0}>
        <Tooltip.Trigger
          as="button"
          class="w-7 h-7 rounded-lg flex items-center justify-center transition-all hover:bg-surface-overlay active:scale-85 md:active:scale-90 cursor-pointer relative z-10"
          aria-label="View context usage"
        >
          <svg width="18" height="18" viewBox="0 0 18 18">
            <title>Context usage indicator</title>
            {/* Background ring (empty state) - stays muted */}
            <circle cx="9" cy="9" r="7" fill="none" stroke="var(--theme-text-muted)" stroke-width="2" opacity="0.3" />

            {/* Filled ring - gradient becomes solid accent on hover */}
            <circle
              cx="9"
              cy="9"
              r="7"
              ...
              class="group-hover:!stroke-accent transition-colors"
              stroke-width="2"
