// ABOUTME: Agent top bar experiment - Swapped Effects: Gradient Pulse for Working, Border for Click
// ABOUTME: Working state uses prominent gradient pulse, click shows border feedback with 5s fade
...
export const metadata = {
  id: "56",
  title: "Swapped Effects: Gradient Pulse for Working, Border for Click",
  description:
    "Working state uses prominent pulsing gradient. Click feedback shows border that fades over 5s. Minimal hover effect.",
};
    ...
    <div class="space-y-4">
      ...
      <div class="flex flex-col h-[600px] bg-surface-raised rounded-lg overflow-hidden border border-border">
        ...
        >
          ...
          <span class="text-sm font-medium bg-gradient-to-r from-gradient-from to-gradient-to bg-clip-text text-transparent group-hover:brightness-110 transition-all relative z-10">
            {title()}
          </span>
          {/* Model name subtle hover */}
          <span class="text-xs text-text-secondary ml-auto mr-2 group-hover:text-text transition-colors relative z-10">
            {mockAgent.modelName}
          </span>
      ...
      <style>{`
        ...
        /* Prominent gradient pulse animation for working state */
        ...
        /* Click feedback border that fades over 5s */
        ...
        .click-border-fade {
          animation: border-fade 5s ease-out forwards;
          box-shadow: inset 0 0 0 2px var(--theme-accent);
        }
      ...
      <div class="bg-surface-raised border border-border rounded-lg p-4 text-sm space-y-2">
        ...
        <ul class="list-disc list-inside space-y-1 text-text-secondary">
          <li>
            <strong>Working State:</strong> Now uses the PROMINENT gradient pulse effect (opacity 0.7 → 0.3 → 0.7) taken
            from experiment #55's hover state. Much more visible than the old subtle pulse.
          </li>
          <li>
            <strong>Click Feedback:</strong> Clicking the top bar shows the border effect (inset accent ring) that fades
            out over exactly 5 seconds. This gives clear tactile feedback.
          </li>
          <li>
            <strong>Hover State:</strong> Now has a minimal effect - just shows a very subtle gradient overlay (15%
            opacity) to indicate interactivity without competing with working/click states.
          </li>
          <li>
            <strong>Visual Hierarchy:</strong> Working state is most prominent (pulsing gradient), click feedback is
            clear but temporary (fading border), hover is subtle (slight glow).
          </li>
          <li>
            <strong>Implementation:</strong> All effects are properly isolated - working uses gradient overlay, click
            uses border animation, hover uses minimal gradient.
          </li>
        </ul>
      </div>
    </div>
