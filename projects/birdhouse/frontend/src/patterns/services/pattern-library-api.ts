...
/**
 * Reveal a pattern group in the system file manager (Finder/Explorer/etc)
 */
export async function revealPatternGroup(encodedGroupId: string): Promise<void> {
  const url = `${API_ENDPOINT_BASE}/pattern-groups/${encodedGroupId}/reveal`;

  const response = await fetch(url, { method: "POST" });

  if (!response.ok) {
    const text = await response.text();
    throw new Error(`Failed to reveal pattern group: ${response.statusText} - ${text}`);
  }
}
