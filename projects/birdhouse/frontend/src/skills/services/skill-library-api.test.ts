...
describe("fetchSkillLibrary", () => {
      ...
      json: async () => ({
        skills: [
          ...
          },
          {
            ...
            metadata_trigger_phrases: ["look up docs"],
  ...
  it("throws a useful error when the skills request fails", async () => {
    ...
    await expect(fetchSkillLibrary("ws_test")).rejects.toThrow(
  ...
  it("loads a single skill detail by encoded skill name", async () => {
    vi.mocked(fetch).mockResolvedValueOnce({
      ...
      json: async () => ({
        ...
        scope: "workspace",
        trigger_phrases: ["spotlight this branch"],
        metadata_trigger_phrases: ["use worktree", "spotlight branch"],
        readonly: true,
    ...
    const result = await fetchSkill("git/spotlight-worktree", "ws_test");
