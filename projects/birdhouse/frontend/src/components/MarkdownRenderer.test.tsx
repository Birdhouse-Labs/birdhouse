...
describe("MarkdownRenderer", () => {
  it("renders Birdhouse model references as plain inline references", () => {
    render(() => <MarkdownRenderer content="Use [openai/gpt-5.4](birdhouse:model/openai/gpt-5.4) here." />);

    const reference = screen.getByText(/openai\/gpt-5.4/i).closest("span");
    expect(reference).not.toBeNull();
    expect(reference?.hasAttribute("data-model-link")).toBe(true);
  ...
  it("does not send model reference clicks through the global reference callback", () => {
    ...
    screen.getByText(/openai\/gpt-5.4/i).click();
