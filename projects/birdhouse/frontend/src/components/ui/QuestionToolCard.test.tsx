...
describe("QuestionToolCard - interrupted state (status=running + isSessionKnownIdle=true)", () => {
  it("shows interrupted indicator when status=running and isSessionKnownIdle=true", () => {
    render(() => (
      <QuestionToolCard
        block={makeBlock()}
        agentId="agent-1"
        pendingQuestions={() => [makePendingQuestion()]}
        isSessionKnownIdle={true}
      />
    ));
    expect(screen.getByText("Question interrupted")).toBeInTheDocument();
  });

  it("does not show submit button when interrupted", () => {
    render(() => (
      <QuestionToolCard
        block={makeBlock()}
        agentId="agent-1"
  ...
  it("shows form when status=running and isSessionKnownIdle=false", () => {
    render(() => (
      <QuestionToolCard
        block={makeBlock()}
        agentId="agent-1"
        pendingQuestions={() => [makePendingQuestion()]}
        isSessionKnownIdle={false}
      />
    ));
  ...
  it("shows form when status=running and isSessionKnownIdle is omitted", () => {
    render(() => (
      <QuestionToolCard block={makeBlock()} agentId="agent-1" pendingQuestions={() => [makePendingQuestion()]} />
    ));
    expect(screen.queryByText("Question interrupted")).toBeNull();
  ...
  it("shows completed summary even when isSessionKnownIdle=true", () => {
    const block = makeBlock({
      status: "completed",
      output: JSON.stringify({ answers: [["Red"]] }),
    });
    render(() => <QuestionToolCard block={block} agentId="agent-1" isSessionKnownIdle={true} />);
    expect(screen.queryByText("Question interrupted")).toBeNull();
    expect(screen.getByText("Red")).toBeInTheDocument();
  });

  it("shows pending spinner even when isSessionKnownIdle=true", () => {
    render(() => (
      <QuestionToolCard block={makeBlock({ status: "pending" })} agentId="agent-1" isSessionKnownIdle={true} />
    ));
    expect(screen.queryByText("Question interrupted")).toBeNull();
    const spinner = document.querySelector(".animate-spin");
    expect(spinner).not.toBeNull();
