...
export const AgentHeader: Component<AgentHeaderProps> = (props) => {
  ...
  const [sessionStatus, { refetch: refetchStatus }] = createResource(
    ...
    async ({ agentId, workspaceId }) => {
      try {
        const response = await fetch(`${API_ENDPOINT_BASE}/workspace/${workspaceId}/agents/${agentId}/status`, { credentials: "include" });
  ...
  const handleExportClick = async () => {
    ...
    try {
      const response = await fetch(buildWorkspaceUrl(props.workspaceId, `/agents/${props.agentId}/export`), { credentials: "include" });
