...
describe("GET /api/agents - Load all agent trees", () => {
  ...
  test("uses resolver aggregate statuses instead of the default harness directly", async () => {
    const agentsDB = await initAgentsDB(":memory:");
    const now = Date.now();

    createRootAgent(agentsDB, {
      id: "agent_alt_status_root",
      session_id: "ses_alt_status_root",
      title: "Alternate Status Root",
    ...
    alternateHarness.seedSessionStatus("ses_alt_status_root", { type: "retry" });
  ...
  test("resolves single-agent status through the agent harness type", async () => {
    const agentsDB = await initAgentsDB(":memory:");
    const now = Date.now();

    const agent = createRootAgent(agentsDB, {
      id: "agent_alt_status_single",
      session_id: "ses_alt_status_single",
      title: "Alternate Status Single",
      harness_type: "alternate",
      created_at: now,
      updated_at: now,
    });
    ...
    await withDeps(deps, async () => {
      const app = await withWorkspaceContext(createAgentRoutes, { agentsDb: agentsDB });
      const res = await app.request(`/${agent.id}/status`);
