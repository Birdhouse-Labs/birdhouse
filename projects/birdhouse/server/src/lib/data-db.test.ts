...
// ABOUTME: Verifies workspace CRUD operations and encrypted secrets storage
...
describe("DataDB", () => {
  ...
  describe("Workspace configuration (encrypted secrets)", () => {
    const testWorkspace = {
      workspace_id: "ws_test123",
      directory: "/test/path",
      opencode_port: null,
  ...
  describe("Low-level secrets operations", () => {
    ...
    test("setWorkspaceSecrets stores encrypted blob", () => {
      const blob = Buffer.from("test encrypted data");
      dataDb.setWorkspaceSecrets("ws_test123", blob);
