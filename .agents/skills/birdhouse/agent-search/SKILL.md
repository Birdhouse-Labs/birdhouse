...
- The search script is bundled with this skill at `scripts/search-messages.ts`
- When the skill loads, note the `Base directory` — use it to construct the absolute script path
...
When this skill loads, a `Base directory` line is injected. Use it to construct the script path:

```bash
# Search a single workspace
bun <SKILL_BASE_DIR>/scripts/search-messages.ts ws_abc123 "search term" --limit 10

# Search ALL workspaces
bun <SKILL_BASE_DIR>/scripts/search-messages.ts all "search term" --limit 10
```
...
```bash
bun <SKILL_BASE_DIR>/scripts/search-messages.ts all "gh pr create" --limit 5
```
